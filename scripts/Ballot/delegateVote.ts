...
async function main() {
  ...
  if (process.argv.length < 4) throw new Error("User address missing");
  ...
  if (userAddress === "")
    throw new Error("The user address is empty");

  console.log("Delegating a vote");
  const transaction = await ballotContract.delegate(userAddress);
